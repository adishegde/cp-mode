...
export default class CpModeView {
    ...
    constructor(serializedState) {
        ...
        inputView = atom.views.getView(this.inputBuffer)
        inputView.style = "height:200px;overflow:scroll"
        input_container.appendChild(inputView)
    ...
    runCode() {
        ...
        var fs = require('fs');
        fs.writeFile(atom.workspace.getActivePane().activeItem.getPath()+"input.txt", this.inputBuffer.getText(), function(err) {
            if (err) {
        ...
        ce.compileCode().then(
            function() {
                return ce.executeCode(path.normalize(atom.workspace.getActivePane().activeItem.getPath()+'input.txt'));
            }
        ).then(
            function(obj) {
                console.log("stdout:\n" + obj);
